...
async function readMembers(filePath) {
  ...
  try{
    const members = await fs.readFile(filePath, 'utf-8'); // 讀取資料
    return JSON.parse(members);

  }catch(error){
    console.log('讀取資料時發生錯誤:', error.message);
    throw error;
  }
}
...
async function getVIPSummary(filePath) {
  ...
  const allMember = await readMembers(filePath); // 1. 讀取會員資料
  const vipMember = filterVIP(allMember); //2. 篩出 VIP
  const count = vipMember.length; // VIP 會員數
  const names = vipMember.map(member => member.name); // VIP 會員姓名
  const totalCredits = vipMember.reduce((acc, member) => acc + (member.credits || 0), 0); // VIP 會員點數
  return {
    count,
    totalCredits,
    names
